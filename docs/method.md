...
The `grilling` skill by Matt Pocock (MIT, Copyright (c) 2026 Matt Pocock) is the source of the interview method in `references/interview.md`. Its design tree, its frontier of questions whose prerequisites are settled, its recommended-answer format, and its rule that the agent finds facts while the user makes decisions are its ideas, not ours.

Earlier revisions called that skill by name and required it to be installed. It is now adapted into this package instead, so nothing has to be installed alongside it. The wording in `references/interview.md` is ours, and three of grilling's rules are deliberately changed: grilling asks the whole frontier per round, whereas this skill asks one question per round and waits for its answer using the host-specific lifecycle (a live run on 2026-09-11 showed a six-question batch with recommendations being treated as already decided — the agent never waited for an answer); grilling ends when the frontier is empty, whereas a Big may be written while questions affecting only individual tasks stay open; and grilling's per-round numbering is transcribed into stable question IDs rather than used as one. Its text is not reproduced; the ideas are credited here and in that file.
...
GitHub's Spec Kit (MIT) is the one external tool this skill calls by name, and only when a repository already carries it. Its `speckit.*` commands write the spec, plan, and task files at the steps `references/speckit.md` maps; its constitution is read as a source with authority; its `clarify` questions are transcribed into the interview's stable IDs rather than asked in its own numbering. Nothing from Spec Kit is bundled, and the workflow runs unchanged without it. The Big and the Minis are not Spec Kit terms, and Spec Kit's `[X]` marks in `tasks.md` are not accepted as verification — the evidence rule above still stands.
