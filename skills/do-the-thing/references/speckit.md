...
GitHub's [Spec Kit](https://github.com/github/spec-kit) is a spec-driven development toolkit: `specify init` drops a `.specify/` folder and a set of `speckit.*` agent commands into a repository, and each command writes one artifact under `specs/NNN-feature/`. When a repository already carries it, this skill calls those commands at every step where a Spec Kit artifact is the natural output. Spec Kit produces the files; the work record stays the record of what was asked, decided, and verified. Nothing here replaces the Big, the Minis, or the checkpoint — the Spec Kit files become their evidence.
...
Pass the work record's identifier through `--short-name` and, when the team numbers features, `--number`, so the feature directory and branch carry the issue key. Read every path the JSON returns as absolute; do not guess a directory from the branch name.
...
| 3 Split | `/speckit.taskstoissues` | Off by default. The work record already holds the work; opening GitHub issues duplicates it. Use only when the user asks for GitHub issues explicitly. |
